...
html2text -nobs sas/output_cl_st1_mariana/loadtable.html > a
...
head -1  sas/output_cl_st1_mariana/cl_st1_mariana_scores.tsv | tr -d '\r' | tr '\t' '\n' > vars
...
do
  column=$( echo " $i + 1 " | bc ) 
  cut -f1,"$column"  sas/output_cl_st1_mariana/cl_st1_mariana_scores_only.tsv | tail +2 > a
  ...
  do
    ...
    do
      ...
      grep -m1 $file  sas/output_cl_st1_mariana/cl_st1_mariana_scores.tsv | tr -d '\r' | tr '\t' '\n' > scores # var values for this text, incl. 0
